...
let make_obsolete =
  Funcall.Wrap.("make-obsolete" <: Symbol.t @-> nil_or Symbol.t @-> string @-> return nil)
;;

let make_function_obsolete obsolete ~current ~since = make_obsolete obsolete current since
