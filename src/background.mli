...
(** [don't_wait_for here f] is like [Async.don't_wait_for], except that it marks all jobs
    created by running [f] as background jobs. *)
val don't_wait_for : ?here:Stdlib.Lexing.position -> (unit -> unit Deferred.t) -> unit
...
module Clock : sig
  (** [every'] is like [Async.Clock.every'], except that it marks the repeating jobs as
      background jobs. *)
  val every'
...
val assert_foreground : ?message:Sexp.t -> ?here:Stdlib.Lexing.position -> unit -> unit
...
module Private : sig
  val mark_running_in_background : ?here:Stdlib.Lexing.position -> (unit -> 'a) -> 'a
  val mark_running_in_foreground : (unit -> 'a) -> 'a
