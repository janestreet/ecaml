...
let mark_running_in_background ?(here = Stdlib.Lexing.dummy_pos) f =
...
let don't_wait_for ?(here = Stdlib.Lexing.dummy_pos) f =
...
module Clock = struct
  ...
  let every ?start ?stop ?continue_on_error ?(here = Stdlib.Lexing.dummy_pos) interval f =
    mark_running_in_background ~here (fun () ->
...
let assert_foreground ?message ?here:(assertion_failed_at = Stdlib.Lexing.dummy_pos) () =
