...
let emacs_of_ocaml (t : Type.t) ocaml_value =
  match t with
  | Int    -> sprintf {|active_env->make_integer(active_env, Long_val(%s))|} ocaml_value
  | Float  -> sprintf {|active_env->make_float(active_env, Double_val(%s))|} ocaml_value
  | Bool   -> sprintf {|(Bool_val(%s) ? emacs_t() : emacs_nil)|} ocaml_value
...
let ocaml_of_emacs (t : Type.t) =
  match t with
  | Int    -> {|Val_long(active_env->extract_integer(active_env, emacs_ret))|}
  | Float  -> {|caml_copy_double(active_env->extract_float(active_env, emacs_ret))|}
  | Bool   -> {|Val_bool(active_env->is_not_nil(active_env, emacs_ret))|}
...
let c ?ocaml_name symbol t_input t_return =
  let ocaml_name = ocaml_name_arg ocaml_name symbol in
  let stub_name = stub ocaml_name in
  ...
  print_endline (
    concat
      ...
      ; "  cache_symbol_and_keep_alive(&emacs_fun, \"";symbol;"\");\n"
      ; convert_args
      ; "  emacs_value emacs_ret = funcall(emacs_fun, ";nargs;", ";args;");\n"
