...
static inline emacs_value intern(const char *symbol_name) {
  return active_env->intern(active_env, symbol_name);
}

static inline emacs_value funcall(emacs_value fun, ptrdiff_t nargs, emacs_value *args) {
  return active_env->funcall(active_env, fun, nargs, args);
}
...
/* We suspect we can only use active_env->free_global_ref from inside the emacs thread, so
...
static emacs_value cache_symbol_and_keep_alive(emacs_value *cache, char* symbol) {
  if (*cache == NULL) {
    *cache = active_env->make_global_ref(active_env, intern(symbol));
  }
  return *cache;
}

/* [emacs_t] is used in generated stubs when an input argument is an OCaml [bool] in order
   to avoid unnecessarily allocation of [emacs_value]'s pointing to [t]. */
emacs_value emacs_t() {
  static emacs_value cache = NULL;
  return cache_symbol_and_keep_alive(&cache, "t");
}
...
static value ocaml_of_emacs(emacs_value val) {
  CAMLparam0();
  CAMLlocal1(v);
  if (val == emacs_nil) {
    v = ecaml_nil();
    ...
    EMACS_OF_OCAML(v) = active_env->make_global_ref(active_env, val);
  }
  CAMLreturn(v);
}
...
ecaml_non_local_exit_get_and_clear(value unit)
{
  ...
  case emacs_funcall_exit_signal:        // Signal (symbol, data)
    symbol = ocaml_of_emacs(non_local_exit_symbol);
    data = ocaml_of_emacs(non_local_exit_data);
    ret = caml_alloc(2, 0);
...
/* This should be used in every place where Emacs calls OCaml and we use [caml_callback]
   ...
   C. */
static void if_exception_signal_emacs_and_use_ecaml_nil(char* symbol_string, value *ret) {
  if(Is_exception_result(*ret)) {
    ...
    *ret = ecaml_nil();
    if (active_env != NULL) {
      ...
      active_env->non_local_exit_signal(active_env,
                                        intern("uncaught-ocaml-exception"),
...
static emacs_value
Fdispatch_assuming_lock_is_held(ptrdiff_t nargs, emacs_value args[], void *data)
{
  CAMLparam0();
          ...
          void *data)
{
  ...
  ret = Fdispatch_assuming_lock_is_held(nargs, args, data);
...
void free_caml_cb(void *ptr)
{
...

  {
    ...
    if_exception_signal_emacs_and_use_ecaml_nil("free_caml_db", &ret);
...
ecaml_make_function(value doc, value fun_id)
{
  ...
  ret_func = ocaml_of_emacs(emacs_func);
  ret_uptr = ocaml_of_emacs(uptr);
...
value ecaml_funcall(value fun, emacs_value *args, size_t nargs, value should_return_result)
{
  ...
  emacs_value ret_val = funcall(the_fun, nargs, args);
  if (Bool_val(should_return_result)) {
    ret = ocaml_of_emacs(ret_val);
  } else {
    ret = ecaml_nil();
  }
  CAMLreturn(ret);
...
{
  emacs_value the_args[3];
  the_args[0] = active_env->make_integer(active_env, Long_val(arg0));
  the_args[1] = active_env->make_integer(active_env, Long_val(arg1));
  the_args[2] = EMACS_OF_OCAML(arg2);
